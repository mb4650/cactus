...
renderPlotly({
  ...
  plot_ly(
    x= ~date) %>% 
    add_lines(y = ~confirmed, color = I("red")) %>%
    add_lines(y = ~deaths, color = I("green")) %>%
    add_lines(y = ~hosp, color = I("blue")) 
...
renderPlotly({
  plot_df %>%
    filter(
      month == input[["month_sel"]],
      day == input[["day_sel"]]
    ) %>%
  plot_geo(locationmode = 'USA-states')%>% 
  add_trace(
    z = ~input[["variable_sel"]], 
    locations = ~key_alpha_2,
    color = ~input[["variable_sel"]], 
    colors = 'viridis'
  ) %>% 
