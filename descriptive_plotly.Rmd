...
us_events_df %>%  
  ...
  geom_text(data = events_df, mapping = aes(x = date, y = 127500, label = event), size = 3.5, angle = 90, vjust = -0.4, hjust = 0)
```
...
us_events_df %>%  
  ...
  geom_point() +
    labs(title = "New deaths over time",
       ...
       x = "date") +
      geom_vline(data = events_df, mapping = aes(xintercept = date) , linetype = "dotted", color = "blue", show.legend = TRUE) +
  geom_text(data = events_df, mapping = aes(x = date, y = 1750, label = event), size = 3.5, angle = 90, vjust = -0.4, hjust = 0)
