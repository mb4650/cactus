...
```{r cross validation, echo = FALSE, message=FALSE, warning=FALSE, results='hide'}
count_cv =
...
```{r correlation 2, echo = FALSE, message=FALSE, results='hide'}
...
```{r regression model 2, echo = FALSE, message=FALSE, results='hide'}
poisson_mod_policy = glm(new_cases ~  month_name + state_name + as.factor(school_closing) + as.factor(workplace_closing) + as.factor(gatherings_restrictions) + state_tavg,
                    family = "poisson"(link = "log"), data = covid_noaa_dataset)
...
```{r cross validation 2, echo = FALSE, message=FALSE,  warning=FALSE, results='hide'}
count_cv =
  crossv_mc(ct_school_work_gather, 100)

count_cv =
  count_cv %>%
  mutate(  
    poisson_policy = map(train, ~glm(new_cases ~  month_name + state_name + as.factor(school_closing) + as.factor(workplace_closing) + as.factor(gatherings_restrictions) + state_tavg,
                    family = "poisson"(link = "log"), data = ct_school_work_gather)),
    neg_binomial_policy = map(train, ~ glm.nb(new_cases ~  month_name + state_name + as.factor(school_closing) + as.factor(workplace_closing) + as.factor(gatherings_restrictions) + state_tavg,
                                             data = ct_school_work_gather))
    ) %>%
  
  mutate(
    rmse_poisson = map2_dbl(poisson_policy, test, ~rmse(model = .x, data = .y)),
    rmse_negative_binomial = map2_dbl(neg_binomial_policy, test, ~rmse(model = .x, data = .y))
    )
...
```{r 2, echo = FALSE, message=FALSE, results='hide'}
...
```{r creating data 4, echo = FALSE, message=FALSE, results='hide'}
ct_infoc_test_tract = 
  covid_noaa_dataset %>%
...
```{r correlation 3, echo = FALSE, message=FALSE, results='hide'}
set.seed(2)
covid_noaa_dataset_cor_3 =
ct_infoc_test_tract %>%
  dplyr::select(new_cases, state_tavg, information_campaigns, testing_policy, contact_tracing)

  covid_noaa_dataset_cor_3[,3:7] %>%
  cor() %>%
  corrplot::corrplot(type = "lower",
                     method = "square",
                     addCoef.col = "red",
...
```{r regression model 3, echo = FALSE, message=FALSE, results='hide'}
poisson_mod_public_health = glm(new_cases ~  month_name + state_name + as.factor(information_campaigns) + as.factor(testing_policy) + as.factor(contact_tracing) + state_tavg,
                    family = "poisson"(link = "log"), data = ct_infoc_test_tract)

poisson_mod_public_health %>%
  broom::glance() %>%
  dplyr::select(AIC) %>%
  knitr::kable(digits = 3)
...
```{r cross validation 3, echo = FALSE, message=FALSE,  warning=FALSE, results='hide'}
set.seed(3)
count_cv_3 =
  crossv_mc(ct_infoc_test_tract, 100)

count_cv_3 =
  count_cv_3 %>%
  mutate(  
    poisson_public_health = map(train, ~glm(new_cases ~  month_name + state_name + as.factor(information_campaigns) + as.factor(testing_policy) + as.factor(contact_tracing) + state_tavg,
                    family = "poisson"(link = "log"), data = ct_infoc_test_tract)),
    neg_binomial_public_health = map(train, ~ glm.nb(new_cases ~  month_name + state_name + as.factor(information_campaigns) + as.factor(testing_policy) + as.factor(contact_tracing) + state_tavg,
                            data = ct_infoc_test_tract))
    ) %>%
  mutate(
    public_health_rmse_poisson = map2_dbl(poisson_public_health, test, ~rmse(model = .x, data = .y)),
    public_health_rmse_nb = map2_dbl(neg_binomial_public_health, test, ~rmse(model = .x, data = .y))
    )

count_cv_3 %>%
  dplyr::select(starts_with("public")) %>%
  pivot_longer(
    everything(),
    names_to = "model",
    values_to = "rmse",
...
```{r 3, echo = FALSE, message=FALSE, results='hide'}
