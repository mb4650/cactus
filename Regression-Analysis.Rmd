...
count_cv =
  count_cv %>%
  mutate(  
    model_poisson_policy = map(train, ~glm(new_cases ~  month_name + state_name + as.factor(school_closing) + as.factor(workplace_closing) + as.factor(gatherings_restrictions) + state_tavg,
                    family = "poisson"(link = "log"), data = covid_noaa_dataset)),
    
    model_neg_binomial_policy = map(train, ~ glm.nb(new_cases ~  month_name + state_name + as.factor(school_closing) + as.factor(workplace_closing) + as.factor(gatherings_restrictions) + state_tavg,
                            data = covid_noaa_dataset))) %>%
  mutate(rmse_poisson = map2_dbl(model_poisson_policy, test, ~rmse(model = .x, data = .y)),
         rmse_negative_binomial = map2_dbl(model_neg_binomial_policy, test, ~rmse(model = .x, data = .y)))
